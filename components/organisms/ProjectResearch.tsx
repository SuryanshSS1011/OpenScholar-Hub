// @/components/organisms/ProjectResearch.tsx
...
interface ProjectResearchPaper {
  id: string;
  researchId: string;
  title: string;
  addedBy: string;
...
const ProjectResearch: React.FC<ProjectResearchProps> = ({ 
  projectId, 
  projectTitle, 
  projectTags = [] 
}) => {
  const { user } = useAuth();
  const [relatedResearch, setRelatedResearch] = useState<ProjectResearchPaper[]>([]);
  ...
  useEffect(() => {
    const fetchRelatedResearch = async (): Promise<void> => {
      if (!projectId) return;
  ...
  const handleAddResearch = async (paper: SearchResultPaper): Promise<void> => {
  ...
  const isPaperAlreadyAdded = (paperId: string): boolean => {
    return relatedResearch.some(r => r.researchId === paperId);
  };
            ...
            {relatedResearch.map((paper) => (
              <div key={paper.id} className="border-b border-gray-100 pb-3 last:border-b-0 last:pb-0">
                ...
                <div className="text-xs text-gray-500 ml-6 flex items-center justify-between">
                  <span>Added {paper.addedAt ? new Date(paper.addedAt).toLocaleDateString() : 'Unknown date'}</span>
                  <Link
                    href={`/research/article/${paper.researchId}`}
                      ...
                      <div className="flex justify-between">
                        ...
                        <div className="ml-4 flex-shrink-0">
                          <button
                            ...
                            disabled={isPaperAlreadyAdded(paper.id)}
                            className={`inline-flex items-center px-3 py-1 rounded text-sm font-medium 
                              ${
                                isPaperAlreadyAdded(paper.id)
                          ...
                          >
                            ...
                            {isPaperAlreadyAdded(paper.id) ? 'Added' : 'Add'}
                          </button>
                        </div>
                      </div>
                    </div>
