// @/components/organisms/ArticleDetail.tsx
...
import { ScholarArticle, CitationData } from '@/types';
...
interface DetailedArticle extends ScholarArticle {
  author?: {
    names: string;
  };
  description?: string;
...
const ArticleDetail: React.FC<ArticleDetailProps> = ({ articleId, onTitleLoad }) => {
  const [article, setArticle] = useState<DetailedArticle | null>(null);
  const [citations, setCitations] = useState<CitationData | null>(null);
  const [relatedArticles, setRelatedArticles] = useState<ScholarArticle[]>([]);
  ...
  const fetchArticleData = useCallback(async (): Promise<void> => {
    if (!articleId) return;
    ...
    try {
      ...
      const articleData: DetailedArticle = await articleRes.json();
      ...
      if (citationsRes.ok) {
        const citationsData: CitationData = await citationsRes.json();
      ...
      if (user) {
        ...
        const isSaved = localStorage.getItem(`saved_${articleId}_${user.id}`);
        setSaved(!!isSaved);
      }
    } catch (err) {
      console.error('Error fetching article:', err);
      const errorMessage = err instanceof Error ? err.message : 'An error occurred while retrieving the article';
  ...
  const handleSave = async (): Promise<void> => {
    ...
    if (!saved) {
      localStorage.setItem(`saved_${articleId}_${user.id}`, 'true');
    } else {
      localStorage.removeItem(`saved_${articleId}_${user.id}`);
    }
  };
    ...
    <div className="bg-white shadow-md rounded-lg overflow-hidden">
      ...
      <div className="p-6">
        ...
        <div className="flex items-center mt-3 mb-4">
          ...
          <span className="text-gray-700">{article.author?.names || article.authors || 'Unknown Author'}</span>
        </div>
                ...
                <ul className="space-y-3">
                  {citations.citingArticles.slice(0, 3).map((cite, index) => (
                    <li key={index} className="border-b border-gray-200 pb-2">
                      ...
                      <p className="text-sm text-gray-500 ml-6 mt-1">{cite.authors || 'Unknown Author'}</p>
                    </li>
                  ))}
                </ul>
            ...
            <div className="space-y-4">
              {relatedArticles.map((related, index) => (
                <div key={index} className="border-l-2 border-blue-100 pl-4">
                  ...
                  <p className="text-sm text-gray-600 mt-1">{related.authors || 'Unknown Author'}</p>
                </div>
              ))}
            </div>
          </div>
