// @/components/organisms/ChatSidebar.tsx
 ...
 */
const ChatSidebar: React.FC<ChatSidebarProps> = ({ activeChannelId, activeDmId }) => {
  const router = useRouter();
  const [isChannelsOpen, setIsChannelsOpen] = useState<boolean>(true);
  const [isDmsOpen, setIsDmsOpen] = useState<boolean>(true);
  const [showCreateChannel, setShowCreateChannel] = useState<boolean>(false);
  const [newChannelName, setNewChannelName] = useState<string>('');
  const [newChannelIsPrivate, setNewChannelIsPrivate] = useState<boolean>(false);
  const [isCreatingChannel, setIsCreatingChannel] = useState<boolean>(false);
  const [createChannelError, setCreateChannelError] = useState<string | null>(null);
  
  const toggleChannels = (): void => setIsChannelsOpen(!isChannelsOpen);
  const toggleDms = (): void => setIsDmsOpen(!isDmsOpen);
  ...
  const handleCreateChannel = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    e.preventDefault();
  ...
  const handleCreateChannelClick = (e: React.MouseEvent<HTMLButtonElement>): void => {
    e.stopPropagation();
    setShowCreateChannel(true);
  };

  const handleDirectMessageClick = (e: React.MouseEvent<HTMLButtonElement>): void => {
    e.stopPropagation();
